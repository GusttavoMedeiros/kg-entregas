...
(async () => {
  ...
  cached.set('https://kg-entregas.vercel.app/ios-like.css?v=3', new Response('estilo-pre-cache'));
  global.fetch = async () => { throw new Error('offline'); };
  assert.equal(await (await requestApp('?v=52')).text(), 'pre-cache');
  assert.equal(await (await requestEstilo('?v=3')).text(), 'estilo-pre-cache');
  ...
  const removidos = [];
  global.caches.keys = async () => ['kg-v22-assets', 'kg-v22-data', 'kg-v23-assets', 'kg-v23-data', 'kg-v24-assets', 'kg-v25-assets', 'kg-v26-assets', 'outro-app'];
  global.caches.delete = async key => { removidos.push(key); return true; };
  let ativacao;
  handlers.activate({ waitUntil: promise => { ativacao = promise; } });
  await ativacao;
  assert.deepEqual(removidos.sort(), ['kg-v22-assets', 'kg-v22-data', 'kg-v23-assets', 'kg-v24-assets', 'kg-v25-assets', 'kg-v26-assets']);
  ...
  assert.match(sw, /CACHE_VERSION = 'kg-v27'/);
  assert.match(sw, /\.\/ios-like\.css\?v=3/);
  assert.match(index, /ios-like\.css\?v=3/);
