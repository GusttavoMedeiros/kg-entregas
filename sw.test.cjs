...
(async () => {
  ...
  cached.set('https://kg-entregas.vercel.app/app.js?v=67', new Response('pre-cache'));
  cached.set('https://kg-entregas.vercel.app/ios-like.css?v=7', new Response('estilo-pre-cache'));
  global.fetch = async () => { throw new Error('offline'); };
  assert.equal(await (await requestApp('?v=67')).text(), 'pre-cache');
  assert.equal(await (await requestEstilo('?v=7')).text(), 'estilo-pre-cache');
  assert.equal((await requestApp('?v=68')).status, 503);
  assert.equal((await requestApp('?v=67&outra=1')).status, 503);
  ...
  assert.equal(await (await requestApp('?v=68')).text(), 'nova');
  ...
  assert.match(sw, /CACHE_VERSION = 'kg-v44'/);
