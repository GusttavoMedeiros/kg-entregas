...
(async () => {
  ...
  cached.set('https://kg-entregas.vercel.app/app.js?v=74', new Response('pre-cache'));
  cached.set('https://kg-entregas.vercel.app/ios-like.css?v=7', new Response('estilo-pre-cache'));
  global.fetch = async () => { throw new Error('offline'); };
  assert.equal(await (await requestApp('?v=74')).text(), 'pre-cache');
  ...
  assert.match(sw, /CACHE_VERSION = 'kg-v51'/);
  ...
  assert.match(index, /app\.js\?v=74/);
