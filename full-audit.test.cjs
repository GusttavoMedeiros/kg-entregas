...
test('falha inicial mantém dados e permite repetir sem recarregar a página',async()=>{
 ...
 vm.runInNewContext(trecho('function normalizarPedidos(', 'async function carregarListas(')+trecho('let carregandoDados =', '// SINCRONIZAÇÃO AUTOMÁTICA'),c);
...
test('nomes iguais a propriedades de Object não quebram totais dos relatórios',()=>{
 const c={foiPago:()=>false,dataRealEntrega:p=>p.data_entregue_em};
